...
public class IndustrialFanScenes {

    public static void direction(SceneBuilder builder, SceneBuildingUtil util) {
        CreateSceneBuilder scene = new CreateSceneBuilder(builder);
        scene.title("industrial_fan_direction", "Air flow of Encased Fans");
        scene.configureBasePlate(0, 1, 5);
        scene.world().showSection(util.select().layer(0), Direction.UP);
        scene.idle(5);
        scene.world().showSection(util.select().fromTo(3, 1, 0, 3, 1, 5)
                .add(util.select().position(3, 2, 4)), Direction.DOWN);
        scene.world().showSection(util.select().fromTo(2, 1, 5, 1, 1, 5)
                .add(util.select().position(1, 2, 5)), Direction.DOWN);
        scene.idle(10);

        BlockPos fanPos = util.grid().at(1, 2, 4);
        ...
        Selection reverse = util.select().fromTo(3, 1, 5, 1, 2, 4);
    ...
    public static void processing(SceneBuilder builder, SceneBuildingUtil util) {
        CreateSceneBuilder scene = new CreateSceneBuilder(builder);
        scene.title("industrial_fan_processing", "Processing Items using Encased Fans");
        scene.configureBasePlate(1, 0, 5);
        scene.world().showSection(util.select().layer(0)
                .substract(util.select().position(0, 0, 4)), Direction.UP);
        scene.idle(5);
        scene.world().showSection(util.select().fromTo(5, 1, 2, 6, 2, 2)
                .add(util.select().position(1, 1, 2)), Direction.DOWN);
        scene.idle(25);

        BlockPos catalystPos = util.grid().at(4, 2, 2);

        // blasting start

        ElementLink<WorldSectionElement> catalyst =
                scene.world().showIndependentSection(util.select().position(3, 2, 0), Direction.SOUTH);
        scene.world().moveSection(catalyst, util.vector().of(1, 0, 2), 0);
        scene.world().setBlock(catalystPos, Blocks.LAVA.defaultBlockState(), false);
        scene.idle(10);

        var areaOfEffect = util.select().fromTo(catalystPos.offset(0, -1, -1), catalystPos.offset(-3, 1, 1));
        scene.overlay().showOutlineWithText(areaOfEffect, 80)
        ...
        ElementLink<EntityElement> entityLink = scene.world().createItemEntity(util.vector().centerOf(catalystPos.west(2)
        ...
        scene.world().hideIndependentSection(catalyst, Direction.NORTH);
        scene.idle(15);
        scene.world().setBlock(util.grid().at(3, 2, 0), campfire, false);
        scene.world().setBlock(catalystPos, campfire, true);
        catalyst = scene.world().showIndependentSection(util.select().position(3, 2, 0), Direction.NORTH);
        scene.world().moveSection(catalyst, util.vector().of(1, 0, 2), 0);
        scene.idle(50);

        scene.overlay().showOutlineWithText(areaOfEffect, 60)
        ...
        scene.world().hideIndependentSection(catalyst, Direction.NORTH);
        scene.idle(15);
        scene.world().setBlock(util.grid().at(3, 2, 0), water, false);
        scene.world().setBlock(catalystPos, water, true);
        catalyst = scene.world().showIndependentSection(util.select().position(3, 2, 0), Direction.NORTH);
        scene.world().moveSection(catalyst, util.vector().of(1, 0, 2), 0);
        scene.idle(20);
        ...
        entityLink = scene.world().createItemEntity(util.vector().centerOf(catalystPos.west(2)
                .above(2)), util.vector().of(0, 0.1, 0), stack);
        scene.idle(15);
        scene.world().modifyEntity(entityLink, e -> e.setDeltaMovement(-0.2f, 0, 0));
        scene.overlay().showControls(itemVec, Pointing.DOWN, 20).withItem(stack);
        ...
        scene.overlay().showText(100)
                .colored(PonderPalette.RED)
                .pointAt(util.vector().topOf(catalystPos.east()))
                .placeNearTarget()
                .attachKeyFrame()
        ...
        /*ElementLink<WorldSectionElement> cogs = scene.world().makeSectionIndependent(
                util.select().fromTo(6, 2, 2, 6, 0, 3).add(util.select().fromTo(4, 0, 2, 5, 0, 2)));
        scene.world().modifyKineticSpeed(util.select().position(5, 2, 2), f -> f / 3f);
        scene.world().moveSection(cogs, util.vector().of(0, 1, 0), 15);
        scene.world().moveSection(catalyst, util.vector().of(0, 1, 0), 15);
        scene.world().destroyBlock(catalystPos.east());
        scene.world().showSection(util.select().position(catalystPos.east().above()), Direction.DOWN);
        scene.world().setBlock(catalystPos.above(), Blocks.WATER.defaultBlockState(), false);*/

        ItemStack sand = new ItemStack(Items.SAND);
        ItemStack clay = new ItemStack(Items.CLAY_BALL);

        scene.idle(20);
        BlockPos depos = util.grid().at(3, 5, 2);
        ElementLink<WorldSectionElement> depot =
                scene.world().showIndependentSection(util.select().position(depos), Direction.DOWN);
        scene.world().moveSection(depot, util.vector().of(-1, -4, 0), 0);
        ...
        ElementLink<WorldSectionElement> largeCog =
                scene.world().showIndependentSection(util.select().position(1, 3, 4), Direction.UP);
        ElementLink<WorldSectionElement> belt =
                scene.world().showIndependentSection(util.select().fromTo(3, 4, 1, 1, 4, 3), Direction.DOWN);
        scene.world().moveSection(largeCog, util.vector().of(-1, -3, 0), 0);
        scene.world().moveSection(belt, util.vector().of(-1, -3, 0), 0);
        ElementLink<BeltItemElement> transported =
                scene.world().createItemOnBelt(util.grid().at(3, 4, 3), Direction.SOUTH, sand);
        scene.idle(60);
        ...
        scene.world().setKineticSpeed(util.select().position(1, 3, 4)
                .add(util.select().fromTo(3, 4, 1, 1, 4, 3)), 0);
